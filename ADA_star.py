...
class ADA_star:
    ...
    def heuristic(self,x,y):
        return self.epsilon*(sqrt((x-self.goal[0])**2+(y-self.goal[1])**2)/(self.robot.max_wheel_vel*self.robot.wheelradius)) 
    ...
    def generate_path(self):
        ...
        while len(self.nodes_to_visit)>0 and found_goal==False:
            ...
            if sqrt((self.goal[0]-node.x)**2+(self.goal[1]-node.y)**2)<50: #and abs(self.goal[2]-(node.angle%360))<10 and abs(self.goal[3]-(node.cart_angle%360))<10:
                found_goal=True
                self.path=node.path
                break
